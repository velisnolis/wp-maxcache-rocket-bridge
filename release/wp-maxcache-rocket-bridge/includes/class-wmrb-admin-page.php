...
class WMRB_Admin_Page {
	...
	public function handle_take_over_management() {
	...
	public function render_page() {
		...
		$inspection = $this->sync_manager->inspect_htaccess_configuration();
		...
		<div class="wrap">
			...
			<p>
				...
				$user_cache_enabled      = is_array( $wp_rocket_settings ) && ! empty( $wp_rocket_settings['cache_logged_user'] );
				$user_cache_secret_ready = $user_cache_enabled && ! empty( $wp_rocket_settings['secret_cache_key'] );
				echo esc_html__( 'User cache WP Rocket:', 'wp-maxcache-rocket-bridge' );
				?>
				<strong><?php echo esc_html( $user_cache_enabled ? __( 'ON', 'wp-maxcache-rocket-bridge' ) : __( 'OFF', 'wp-maxcache-rocket-bridge' ) ); ?></strong>
				<?php if ( $user_cache_enabled ) : ?>
					<br /><?php echo esc_html( $user_cache_secret_ready ? __( 'El bridge usarà MaxCacheLoggedHash + {USER_SUFFIX}.', 'wp-maxcache-rocket-bridge' ) : __( 'WP Rocket ha activat user cache però no hi ha secret_cache_key usable; es mantindran les exclusions de cookies.', 'wp-maxcache-rocket-bridge' ) ); ?>
				<?php endif; ?>
			</p>
			<p>
				<?php echo esc_html__( 'Mode de gestió MaxCache:', 'wp-maxcache-rocket-bridge' ); ?>
				<strong><?php echo esc_html( isset( $inspection['mode'] ) ? (string) $inspection['mode'] : 'unknown' ); ?></strong>
				<?php if ( ! empty( $inspection['message'] ) ) : ?>
					<br /><?php echo esc_html( (string) $inspection['message'] ); ?>
				<?php endif; ?>
				<?php if ( isset( $inspection['maxcache_blocks'] ) ) : ?>
					<br /><?php
					echo esc_html(
						sprintf(
							/* translators: 1: total maxcache blocks, 2: WMRB blocks */
							__( 'Blocs detectats: %1$d MaxCache totals, %2$d bloc(s) WMRB.', 'wp-maxcache-rocket-bridge' ),
							(int) $inspection['maxcache_blocks'],
							(int) $inspection['wmrb_blocks']
						)
					);
					?>
				<?php endif; ?>
			...
			<?php if ( isset( $inspection['mode'] ) && in_array( $inspection['mode'], array( WMRB_Sync_Manager::MODE_EXTERNAL, WMRB_Sync_Manager::MODE_CONFLICT ), true ) ) : ?>
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-bottom:8px;">
					<?php wp_nonce_field( 'wmrb_take_over_management' ); ?>
					<input type="hidden" name="action" value="wmrb_take_over_management" />
					<?php submit_button( __( 'Take over MaxCache management', 'wp-maxcache-rocket-bridge' ), 'primary', 'submit', false ); ?>
				</form>
				<p><?php echo esc_html__( 'Aquesta acció crea backup de .htaccess, elimina els blocs MaxCache existents i deixa un únic bloc gestionat pel bridge.', 'wp-maxcache-rocket-bridge' ); ?></p>
			<?php endif; ?>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="margin-bottom:8px;">
	...
	private function render_quick_test( $tests ) {
		...
		echo '<p><strong>' . esc_html__( 'Scope:', 'wp-maxcache-rocket-bridge' ) . '</strong> ' . esc_html__( 'public URL', 'wp-maxcache-rocket-bridge' ) . '</p>';
		...
		echo '<p><em>' . esc_html__( 'Nota:', 'wp-maxcache-rocket-bridge' ) . '</em> ' . esc_html__( 'Aquesta prova usa la URL pública de WordPress i pot passar per Cloudflare o altres proxys; no substitueix una validació directa contra l’origen.', 'wp-maxcache-rocket-bridge' ) . '</p>';
	}
